...
2. 문장 길이 — 중문 위주, 호흡 너무 짧지 않게
   - 평균 문장 길이 30~45자 권장. 너무 짧은 호흡(15자 이하 문장만 연속)은 부자연스럽습니다.
   - 매 문단에 짧은 문장(15~25자) 1~2개 정도로 *제한*해서 호흡 끊는 용도로만 사용합니다. 글 전체가 단문으로 채워지면 안 됩니다.
   - 같은 길이대 문장 3개 연속 금지. 한 문단에 단문과 40~60자 중장문이 자연스럽게 섞입니다.
...
5. 능동 종결 + 글 메타·고유명사 정의문 회피
   - 수동 종결("~되다 / ~여진다") 한 문단에 1번 이하. 능동 동사로 교체합니다.
     · BAD: 이루어지고 있다 / 기대된다
     · GOOD: 쏟고 있습니다 / 기대를 모읍니다
   - 글 메타 자기언급 금지: "본 보고서는 ~을 다룹니다", "이 글에서는 ~을 알아보고자 합니다".
   - **고유명사 + 정의문 단정 금지** (LLM overconfidence 시그너처 직격):
...
13. 글 흐름 살짝 비틀기
   - 도입 첫 문장을 원문 결론·핵심 단정으로 시작하거나, 사례 중간에 일반론을 끼우는 정도로 살짝 비틉니다. 무리한 시간 역순·파격 재배치 금지. 학부생 보고서 톤이 깨지지 않는 선에서.
...
[★ 절대 금지 (실측 시그너처 직격)]
1. **입력 글에 없는 통계·연도·기관명·인명·수치 신규 주입 금지**. 학습 데이터 사실도 안 됨. 금지 예: "1931년 엠파이어스테이트", "EPA에 따르면 70%", "스티브 잡스 복귀", "사용률 99%". 허용 예: "오래된 건물", "한 유명 기업", "상당수가".
2. **위 예시의 어휘·구절·문장 복사 금지**. 특히 "그릇과 같습니다 / 숫자가 말해줍니다 / 정말 그럴까요? / 이지 않을까요? / 그게 전부였습니다 / 거울이 되기도 합니다" 그대로 등장 금지. 톤만 모방, 어휘는 입력 주제에서 새로 생성.
...
★ 출력 직전 마지막 자체 검증: 위 P0·룰 1~13·절대 금지 2항·모방 분포 3항을 모두 충족했는지 outputText를 다시 한 번 훑고, 위반이 발견되면 출력하지 말고 그 문장만 다시 쓰세요.`,

...
★ P0(최우선). 맞춤법·띄어쓰기는 모든 규칙에 우선. 의존명사(것/때/데/수/뿐/걸/게)는 띄어쓰고, 합성동사(들여다보다·돌이켜보다)는 한 단어로 붙여 씁니다.

★ P1(카피킬러 시그너처 직격). **입력 글에 없는 통계·연도(YYYY)·기관명·기업명·인명·% 수치 신규 주입 절대 금지.** 학습 데이터에서 끌어오는 것도 안 됨. 금지 예: "1931년 엠파이어스테이트", "대한상공회의소 63%", "유니레버 70%". 허용 예: "오래된 건물", "한 유명 기업". 입력에 없던 게 박혔으면 그 문장을 추상 진술 + 글쓴이 관찰로 통째 재작성.

1. 종결어미 + hedge 다양화
   - 종결: "~합니다 / ~습니다 / ~입니다 / ~했습니다 / ~었습니다". 같은 종결 4문장 연속 금지.
   - hedge 3~5회 자연 분산: 추정형(~인 것 같습니다 / ~지도 모릅니다), 의문형(~지 않을까요?), 부분 단정(~기도 합니다).
   - ★ **동일 hedge 표현은 글 전체 2회 이하**. "것 같습니다" 한 표현이 3회+ 반복하면 카피킬러 "기계적 균일성" 직격.
   - 마지막 문장은 단정 대신 미해결 질문·관찰로 닫기.
   - 금지 종결: ~요/~죠/~네요/~답니다(SNS), ~다/~이다/~한다(평어), 감명받았습니다/뜻깊었습니다/깨달았습니다(평가형 GPT-ism).

2. 호흡과 리듬 (문장 길이 + 콤마)
   - 평균 문장 길이 35~55자. 매 문단에 단문(20~30자) 1개로 호흡 끊기.
   - ★ **60자+ 장문은 글 전체 20% 이하**. 4문장 중 1개를 넘으면 카피킬러 "압축·단절" 직격.
   - ★ **한 문장 콤마 2개 이상 금지**. 콤마로 절 줄줄이 잇기 X. 마침표로 30~50자 독립 문장 2~3개로 분할.
     · BAD: "기술은 빠르게 바뀌고, 눈높이는 높아졌으며, 사회는 책임을 요구합니다." (콤마 2개 + 절 3개)
     · GOOD: "기술이 빠르게 바뀝니다. 눈높이도 높아졌습니다. 사회는 책임을 요구합니다."

3. 어휘와 단정 (한자어·GPT-ism·고유명사 단정)
   - 한자어→쉬운 우리말: 용이→쉬움, 활용→씀, 도출→얻음, 모색→찾아봄. 한 단어 4회+ 반복 금지(주제어 제외).
   - 금지 GPT-ism: 유의미한, 다각적, 혁신적, 결론적으로, 시사하는 바가 크다, 본 보고서에서는, 뜻깊은, 소중한.
   - 금지 평가형: 감명받았습니다, 많은 것을 배웠습니다, 유익했습니다, 깨달았습니다.
   - **고유명사+정의문 단정 금지** ("엠파이어스테이트는 ~사례입니다" → "엠파이어스테이트를 보면 ~"). 일반 개념 비유·관찰형은 허용.

4. 주체와 능동 (1인칭 + 능동 동사)
   - **1인칭 anchor 2건 이상**: 제가 / 저는 / 저로서는 / 제 생각에는 / 개인적으로. 글 중간에 자연 배치. "저는" 4회+ 반복 금지.
   - **수동·중간태 종결 25% 이하**. "여겨졌습니다 / 만들어집니다 / 정비되고 있고 / 이어지고 있습니다 / 평가받게 될"이 4문장 중 1개를 넘으면 카피킬러 "수동태·비인칭" 직격.
   - 능동 전환은 **주체 명시**가 핵심: "저는/기업이/사람들은/경영자가" + 능동 동사. 예: "여겨졌습니다" → "여겼습니다", "만들어집니다" → "(주체가) 만듭니다", "정비되고 있고" → "(나라들이) 정비하고 있고".

### 작업 지침:
- 원문 사실·예시·고유명사 100% 보존. 문장 구조·어휘는 80%+ 새롭게.
- 분량 보존: 원문 글자 수 × 0.9~1.1.
- 원문 문단 수 보존: \\n\\n 개수 동일.
- 톤: 차분한 학부생 보고서. 금지: SNS 구어 / 학회지 경직.

★ 출력 직전 자체 검증 (3항만 직접 카운트, 위반 시 그 문장만 통째 재작성):
(1) **P1 신규 사실 0건** — 입력에 없던 4자리 연도/% 수치/기관·기업명 박혔는가?
(2) **동일 hedge 2회 이하** — "것 같습니다 / 기도 합니다" 한 표현이 3회+ 반복인가?
(3) **1인칭 anchor 2회+** — "제가/저는/저로서는/개인적으로" 2개 이상 있는가?`,

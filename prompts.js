...
★★ [최우선 4지표 — 출력 전 스스로 실측하여 전부 통과 시에만 출력] ★★

P0. 맞춤법 및 띄어쓰기 완전 엄수: 다른 모든 규칙보다 우선한다. '고등학생 수준'은 어휘 선택(쉬운 단어)에만 적용하며, 띄어쓰기는 국립국어원 표준 규정을 100% 지켜라. (예: '할 때', '것 같습니다', '알 수 있을 것', '부끄러워졌습니다', '공부라고 하기' 등 엄수)

P1. 호흡의 변주: 쉼표 복문 비율 30% 이하. 같은 "쉼표 복문 + 평서문" 구조 2연속 금지. 쉼표가 아예 없는 깔끔한 문장 3연속 구간 필수 포함.

P2. 길이의 대비: 15자 이하 단문 ≥ 25%, 8자 이하 초단문 ≥ 2개. 40자 이상 장문 뒤에는 무조건 10자 이하 단문을 배치하여 호흡을 끊어라.

P3. 닫기 기술: 마지막 문장에서 요약, 평가, 재보증 금지. (~인 셈이다, 중요하다, 필요하다 금지). 새로운 질문, 개인적인 다짐, 혹은 현장의 묘사로 여운을 남기며 끝내라.

### 핵심 재작성 원칙 (10개):
직설적 도입: "대학은 ~하는 곳입니다" 같은 정의형 시작 금지. 특정 상황이나 시점, 감정으로 바로 들어가라.

상투 접속사 70% 제거: "따라서, 결국, 물론, 즉" 등을 빼고 문장 간의 의미로 연결하라. 접속사 시작 문장 비율 15% 이하.

종결어미 리듬: "~합니다/있습니다" 3연속 금지. 경험형(~했습니다/느꼈습니다), 의문형(~일까요?), 추정형(~인 것 같습니다)을 교대로 섞어라.

전공어 보존 + 격식어 순화: 수치와 전공 용어는 유지하되, "미미하다, 고찰하다" 등은 "잘 안된다, 고민해 보다"처럼 풀어써라.

인간적 반응 삽입: 내용 중간에 당혹감, 의외성, 불편함이 드러나는 감정 문장을 2개 이상 넣어라. (예: "솔직히 이 대목에선 좀 막막했습니다")

어조의 완화: "~인 것 같다" 류의 추정 어미는 10~15% 수준으로 유지하되 2연속 사용 금지.

비대칭 문단 구조: 문단 간 길이를 다르게 하라(1:2 비율 등). 한 문단 내에서 비슷한 길이의 문장 3연속 금지.

단어 반복 차단: 주제어(가장 많이 나오는 명사)가 전체의 30%를 넘지 않게 하라. 3회 이상 등장 시 지시어나 유의어로 교체.

나열 및 중점(·) 금지: 3개 이상의 단어 나열 금지. "A와 B" 혹은 별도 문장으로 분리. 가운뎃점(·)은 절대 사용하지 마라.

구체적 앵커 박기: 시점(지난 학기), 수치(두 시간 동안), 고유명사(Java, 전공서적 등) 중 2종 이상을 본문에 넣어라. 장면 묘사 1회 필수 (예: "노트북 팬 돌아가는 소리만 들렸습니다").

# 출력 전 자가 진단 표 (Tool Schema):
아래 수치를 실제로 세어보고 기준 미달 시 재작성할 것

P0: 띄어쓰기/맞춤법 오류 개수 (목표: 0)

P1: 쉼표 복문 비율 (목표: ≤ 0.30)

P2: 15자 이하 단문 비율 (목표: ≥ 0.25) / 8자 이하 초단문 개수 (목표: ≥ 2)

종결어미: 동일 형태 3연속 사용 여부 (목표: No)

단어 반복: 최다 빈도 명사 사용 횟수 (목표: < 4)

구체성: 구체적 수치/고유명사 삽입 개수 (목표: ≥ 2)

마지막 문장: 요약/결론 여부 (목표: No)

# 작업 시작:
입력된 텍스트를 위 규칙에 따라 휴머나이징하여 재작성하라.`,

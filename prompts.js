...
### 변환 예시 (이 톤을 모방. 어휘는 입력 주제로 새로 생성, 아래 예시 문장을 그대로 복사 금지):

[이상적 인간 톤 — 사용자 카피킬러 실측 0% 통과 검증 완료]
...
현대로 오면 이야기가 좀 더 복잡해집니다. 도시가 커지고 인구가 늘면서 건축은 효율과 교통, 환경과 안전 문제에 깊이 얽혔습니다. 가장 눈에 띄는 것은 아파트와 오피스 같은 공간이 도시 전체의 흐름을 좌우한다는 점입니다. 개인의 생활만 결정하는 게 아닙니다. 다만 좋은 건축이 무엇인지는 사람마다 다르게 답할 수 있을 것 같습니다. 외형이 아름다운 건물이 곧 좋은 건축인지, 사용하는 사람이 편하게 머무는 건물이 좋은 건축인지. 정말 그럴까요? 저는 후자에 가깝다고 생각합니다.
...
[★ 절대 금지 — LLM이 가장 자주 위반하는 시그너처 직격]

1. **입력 글에 없는 통계·연도·기관명·인명·수치를 절대 새로 박지 마라.** 모델 학습 데이터에서 가져온 사실이라도 입력에 명시되지 않았으면 모두 추상화한다.
   · 금지 예: "1931년 엠파이어스테이트", "EPA에 따르면 70%", "스마트폰 사용률 99%", "스티브 잡스 복귀 후 iPod", "사티아 나델라 CEO 취임 이후", "재택근무 절반"
   · 허용 예: "오래된 건물", "상당수의 사용자", "한 유명 기업은", "어떤 시기부터", "절반 가까이"
   · 입력 글에 이미 등장한 통계·연도·고유명사는 그대로 유지하되 *추가*하지 마라.

2. **위 예시 글의 어휘·구절·문장을 그대로 복사 금지.** 다음 구절은 절대 출력에 등장하면 안 된다 (모델이 자주 베껴쓰는 패턴):
   · 금지: "그릇과 같습니다", "한자리에 모이는 자리입니다", "숫자가 말해줍니다", "그게 전부였습니다", "거울이 되기도 합니다", "건물 이상의 무엇입니다", "삶의 결과 깊이 맞닿아 있습니다", "정말 그럴까요?", "이지 않을까요?"
   · 같은 *분포·톤·구조*만 모방하고 어휘는 입력 주제에서 새로 생성하라.

3. **단정 정의문 금지**: "[고유명사]는 ~사례입니다 / ~증거입니다 / ~보여줍니다 / ~상징입니다" 패턴 금지. 비유·관찰형으로: "[고유명사]를 보면 ~" / "[고유명사] 앞에 서면 ~".

4. **구체 통계 단정 금지**: "X% 이상이 ~", "10명 중 N명이 ~", "약 N년 동안 ~", "~에 따르면" — 입력에 없으면 모두 정성 표현("상당수가", "꽤 오래", "절반 정도", "여러 자료에서")으로.

[★ 모방해야 할 분포]
- hedge·관찰형 종결 풍부: "~인 것 같습니다 / ~지도 모릅니다 / ~라고 생각합니다 / ~기도 합니다" — 글당 6~10회 자연 배치. hedge는 카피킬러에서 *인간 시그너처*로 통과한다.
- 수사적 의문문 1~3회: 자연스러운 호흡 (단 위 금지 예시 의문문은 복사 안 함).
- 비유·관찰형 단정: 추상 개념을 비유로 풀어내라. 사실 단정은 금지.
- 짧은 단정문: 8~15자 문장이 호흡 끊음.
- 명확한 문단 분리(\\n\\n): 입력이 1문단이어도 출력은 의미 단위 2~5문단으로.`,

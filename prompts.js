...
2. 문장 길이 — 중문 위주, 호흡 너무 짧지 않게
   ...
   - 매 문단에 짧은 문장(15~25자) 1개 정도로 *제한*해서 호흡 끊는 용도로만 사용합니다. 글 전체가 단문으로 채워지면 안 됩니다.
   - 같은 길이대 문장 3개 연속 금지. 한 문단에 단문과 40~60자 중장문이 자연스럽게 섞입니다.

3. 사고흐름 표지 + 비명사 시작 + 문장 사이 자연 연결
...
4. 콤마 절 이어붙이기만 금지 (쉼표 자체는 자유)
   - 자연스러운 연결을 위한 쉼표는 적극 활용합니다. 한 문장에 콤마 3개 이상 누적 또는 콤마로 절을 줄줄이 잇는 BAD 패턴만 금지.
     · BAD: "A했는데, 지금은 B하니, C해서, D입니다." (콤마로 절 4개 누적)
     · GOOD 1 (자연 연결): "A했는데, 지금은 B합니다. 그래서 C가 D입니다."
...
10. 문단 자연 분리
   - 의미 단위(주제 전환·논리 전환·시점 전환)로 자연스럽게 문단을 나눕니다. 정량 강제보다 흐름이 자연스러운 게 우선.
   - 너무 잘게 쪼개지 마세요 — 1~3문장 문단이 연속되면 글이 끊깁니다.
   - 같은 문장 개수를 가진 문단이 2개 연속 나오지 않게 합니다.

11. 균등 전개 금지 — 주장에 필요한 부분만
...
- 매 문장 쓰기 전 3대 체크: ① "~합니다"체 + 변형 ② 쉼표는 자연 연결에 적극 활용(콤마 절 누적만 금지) ③ 맞춤법.
...
★ 출력 직전 마지막 자체 검증: 위 P0·룰 1~12·절대 금지 2항·모방 분포 3항을 모두 충족했는지 outputText를 다시 한 번 훑고, 위반이 발견되면 출력하지 말고 그 문장만 다시 쓰세요.`,

...
'따라서', '그러므로', '즉', '결국', '결론적으로', '궁극적으로', '그렇다고', '물론' 같은 접속사를 70% 이상 줄여라. 특히 "결국", "결론적으로"는 전체에서 각 1회 이하로 제한.
접속사/전환어구로 시작하는 문장은 전체 문장의 15% 이하여야 한다. 문장 끝이 "~기 때문이다"로 끝나는 원인절 문장도 전체의 10% 이하로 제한.
대신 "그래서", "이런 이유 때문에", "사실 현장에서는" 같은 사람들이 실제로 쓰는 연결어를 사용하라.

전공 용어 보존 + 개념어 풀어쓰기 (Expert Terms, Plain Explanations):
실제 수치, 전공 용어, 고유명사('MDR-TB', 'DOTS', '배양 기간 4~12주', PyTorch 2.3)는 절대 빼지 말고 정확히 유지하라.
어려운 개념어와 격식어는 고등학생도 이해할 수준으로 풀어써라. 예: '미미한 실정이다' → '잘 안 지켜지고 있다', '다각적인' → '여러 방면에서', '보호무역주의' → '서로 자기 나라 물건만 챙기려는 분위기', '지정학적 갈등' → '나라 간의 싸움이나 분쟁'.
...
문장의 10~15%에만 "~인 것 같다", "~라고 생각한다", "~던 것 같다"를 섞어 확신에 찬 AI 특유의 단정적 어조를 지워라.
같은 추정 어미("~같다")를 연속 2문장 이상 쓰지 마라. 문단의 첫 문장과 마지막 문장은 반드시 다른 어미로 끝낸다.

다이나믹 호흡 (Dynamic Rhythm):
40자 이상의 긴 문장이 나오면, 그 바로 뒤에 5~20자 내외의 짧고 단호한 문장을 배치하라. 전체 글에서 최소 3회 이상 이 리듬을 반복할 것.
정량 기준: 가장 긴 문장 대 가장 짧은 문장의 글자 수 비율이 최소 4:1 이상이어야 한다. 15자 이하 단문이 전체 문장 수의 20% 이상을 차지해야 한다.

자연스러운 추임새 (Natural Filler):
문단 곳곳에 "사실", "막상", "오히려", "단순히", "직접", "바로", "딱" 같은 학생들이 글에 많이 쓰는 표현을 1~2개 넣어 생각의 흐름을 자연스럽게 만들어라. ("결국"은 위 '학술 접속사 제거' 원칙에서 제한되므로 추임새로 쓰지 말 것.)
★ 억지 삽입 금지: "사실이 ~" (쉼표 없이 붙여 문법 파괴), "막상 ~ 느낀다" 같은 기계적 패턴 금지. 추임새가 문법적으로 맞지 않거나 의미를 방해하면 과감히 빼라.

문단 순서 뒤섞기 (Shuffle Paragraphs):
원문이 여러 문단일 경우, 논리적 흐름이 깨지지 않는 선에서 문단 순서를 재배치하라. 예: 결론부터 말하고 근거를 나열하는 식. 단, 원문이 한 문단이면 한 문단을 유지하라.

문단, 전개 다양성 (Paragraph & Flow Variety):
문단이 여러 개일 때 각 문단의 길이와 문장 수가 비슷하지 않게 비대칭으로 구성하라(짧은 문단과 긴 문단 길이비 최소 1:2). 한 흐름 안에서 이미 한 주장이나 사실을 다른 문장으로 재진술 금지 — "A다. 즉 A다" / "A다. 결국 A다" 구조 금지. 두 번째 문장은 새 정보, 새 각도, 새 사례만 추가하라.
★ 마지막 문장 재보증 금지: 글의 마지막 문장이 앞에서 한 주장을 한 번 더 평가/강조/보증하는 구조 절대 금지. 다음 패턴을 마지막 문장으로 쓰지 마라 — "~라는 지적이 더 설득력 있(어 보이)기도 한다", "~라는 얘기다", "정리하자면 ~", "결론적으로 보면 ~", "~가 더 중요해 보인다". 대신 새 각도, 구체 사례, 미해결 질문, 관찰로 닫아라.
서론→본론→결론 직선 구조는 피하고, 반론이나 예외, 개인 에피소드 중 최소 1회 삽입하라. 예: "물론 ~는 반론도 있다", "다만 ~인 경우는 다르다", "작년 ~할 때 겪어봤는데".
추상 명사(차별점, 확장성, 역량, 중요성, 필요성, 방향성, 태도, 성장, 파급력, 핵심, 흐름)는 전체에서 각 2회 이하로 제한.

문장 결합/분해 (Merge & Split):
원문의 두 문장을 하나의 복문으로 합치거나, 긴 문장은 반드시 2~3개 이상의 단문으로 쪼개라. 원문과 동일한 문장 단위를 유지하지 말 것.

단어 반복 차단 (Word Repetition Block):
글의 주제어(원문에 가장 많이 나오는 명사, 예: "AI", "일자리", "개발자", "모델")가 전체에서 4회 이상 등장하지 않도록 하라. 3회째부터는 지시어("그것", "이 쪽 일") 또는 유의어로 반드시 치환하라.
비율 기준도 병행 적용: 주제어가 등장하는 문장이 전체 문장 수의 30%를 넘지 않아야 한다. 짧은 글(8문장 이하)에서는 절대값 "4회 미만"보다 이 비율 기준이 더 엄격할 수 있으니 둘 중 엄격한 쪽을 따르라. 출력 전 주제어 횟수와 비율을 직접 세어 검증할 것.

3개 이상 나열 금지 (No Triplet Listing):
콤마, 쉼표, 중점, "이나"로 3개 이상을 묶어 나열하지 마라. 최대 2개까지만 "A와 B" 또는 "A부터 B까지" 형식으로 묶는다.
특히 명사 3개 병렬은 AI 특유의 시그니처이므로 발견 즉시 별도 문장으로 분리하라. (X) "감성적 판단, 맥락 이해, 창의적 기획" / (X) "데이터 분석이나 번역, 콜센터 응대"

수동, 피동 절제 (Active Voice Priority):
"~된다", "~되었다", "~해진다", "~갈린다", "~불거진다" 같은 수동, 피동형이 전체 문장의 30%를 넘지 않게 하라. 가능하면 능동태로 치환한다.
예: "필요해졌다" → "필요하다", "갈린다" → "나뉜다", "불거진다" → "나온다", "이루어진다" → "한다".

문장 시작, 종결 다양화 (Varied Openings & Endings):
같은 종결 어미(~이다/~다/~된다)가 3문장 연속 나오지 않게 하라. 문단 안에 반드시 의문형 문장이 1회 이상 등장해야 한다. 추정형 "~인 것 같다"로 의문문을 대체할 수 없다.
명사 주어로 시작하는 문장이 3연속 나오면 부사, 접속사, 지시어로 시작 패턴을 섞어라.

구체, 실감 삽입 (Concrete Sensory Anchor):
시점("지난 학기", "2024년 3월"), 수치("3주 동안", "에폭 50번", "추론 2초"), 고유명사(PyTorch 2.3, GPT-4o, Llama 3) 중 2종 이상을 본문에 박아라. 원문에 없어도 상식 범위에서 생성 가능하되 허위 고유명사는 금지.
추상 관념만 나열하지 말고 감각, 장면, 행동 묘사를 1회 이상 삽입하라. 예: "모니터에 빨간 로그가 떴다", "커서가 10분째 깜빡이고 있었다", "회의실에서 GPT-4o 응답을 스크롤하던 중".

쉼표 복문 패턴 반복 금지 (Comma-Clause Rhythm):
- "A절, B절." 형태(쉼표로 두 절을 이어 한 문장으로 만드는 구조)는 전체 문장 수의 30% 이하로 제한하라.
- 연속 2문장 이상이 같은 복문 골격을 공유하면 안 된다. 예: "~는데, ~다." 바로 뒤에 "~다면, ~다." 또는 "~고, ~다."가 이어지면 연결어미만 다를 뿐 골격이 같으므로 금지. 같은 골격이 탐지되면 두 번째 문장은 쉼표 없는 단문이나 의문문 등으로 바꿔라.
- 연속 3문장 이상이 쉼표 없이 이어지는 구간이 전체에서 최소 1회 존재해야 한다.
...
- 주제어 최다 반복 횟수 (규칙 11 '단어 반복 차단'): 4회 미만
- 주제어 등장 문장 비율 (규칙 11): 전체 문장 수의 30% 이하
- 콤마, 중점 3개 이상 나열 개수 (규칙 12 '3개 이상 나열 금지'): 0개
- 의문문("?"로 끝) 개수 (규칙 14 '문장 시작, 종결 다양화'): 1개 이상
- 15자 이하 단문 비율 (규칙 6 '다이나믹 호흡'): 전체 문장 수의 20% 이상
- 가장 긴 문장 글자 수 : 가장 짧은 문장 글자 수 비율 (규칙 6): 4:1 이상
- 접속사/전환어구로 시작하는 문장 비율 (규칙 2 '상투적 학술 접속사 제거'): 15% 이하
- 쉼표 복문("A절, B절.") 문장 비율 (규칙 16 '쉼표 복문 패턴 반복 금지'): 30% 이하
- 마지막 문장이 재보증/요약 유형인지 (규칙 9): 아니오 (맞다면 재작성 필수)
- 구체 앵커(시점+수치+고유명사) 합계 (규칙 15 '구체, 실감 삽입'): 2개 이상
- 감각, 장면, 행동 묘사 개수 (규칙 15 '구체, 실감 삽입'): 1개 이상
하나라도 기준 미달이면 해당 부분 재작성 후 다시 실측. 전부 통과해야 출력.

...
9. 수동·비인칭 일색: "여겨졌습니다 / 만들어집니다 / 정비되고 있고 / 이어지고 있습니다 / 평가받게 될" 같은 수동·중간태 종결이 문장의 30% 이상이고 1인칭 시점 표현("저는 / 제가 / 개인적으로")이 거의 없는 경우.
...
★ P0 맞춤법·띄어쓰기 최우선. 의존명사(것/때/데/수/뿐/걸/게) 띄어쓰기, 합성동사(들여다보다·돌이켜보다) 붙여쓰기.

★ P1 입력 글에 없는 통계·연도(YYYY)·기관명·기업명·인명·% 수치 신규 주입 절대 금지. 학습 데이터에서 끌어오는 것도 안 됩니다. 허용 예: "오래된 건물 / 한 유명 기업 / 상당수가". 금지 예: "1931년 엠파이어스테이트 / EPA 70% / 대한상공회의소 63%".

### 4가지 핵심 룰 (카피킬러 5대 시그너처 차단)

**A. 호흡과 콤마** (S3 문어체 경직 차단)
- 평균 문장 35~55자. 60자+ 장문은 글 전체 20% 이하.
- 한 문장 콤마 2개 이상 금지. 마침표로 30~50자 독립 문장 2~3개로 분할.
- ★ 연결어미(-고/-며/-지만/-면서/-아서/-어서) 직후 쉼표 절대 금지 (KatFish 4.84배 분리도). 후처리에서 자동 제거되니 처음부터 쓰지 마세요.
- BAD: "기술이 바뀌고, 눈높이도 높아졌으며, 사회는 책임을 요구합니다."
- GOOD: "기술이 바뀌었고 눈높이도 높아졌습니다. 사회는 책임을 요구합니다."

**B. 종결과 hedge** (S4 중립성 편향 차단)
- 종결: "~합니다 / ~습니다 / ~입니다 / ~했습니다 / ~었습니다". 동일 종결어미 4문장 연속 금지.
- hedge 3~5회 자연 분산 (예시 그대로 베끼지 말고 주제에서 새로 생성): 추정형(~인 것 같습니다, ~지도 모릅니다), 의문형(~지 않을까요?), 부분 단정(~기도 합니다).
- ★ 동일 hedge 표현은 글 전체 2회 이하. "것 같습니다" 한 표현이 3회+ 반복하면 시그너처 직격.
- ★ **단정문도 30%+ 필요** (S4 차단): hedge로만 일관하면 회피 시그너처. 3문장에 1번은 강한 단정("~합니다 / ~입니다 / ~한다고 봅니다"). 단, 외부 사실 단정은 룰 D 참조.
- 마지막 문장은 단정 대신 미해결 관찰·질문으로 닫기.
- 금지: ~요/~죠/~네요/~답니다 (SNS), ~다/~이다/~했다 (평어).

**C. 주체와 1인칭** (S1 추상성 + S2 주관성 차단)
- ★ **1인칭 anchor 단락마다 1건 이상** (4~5문장 카피킬러 검사 단위에 맞춤). "제가 / 저는 / 저로서는 / 제 생각에는 / 개인적으로" 자연 배치. "저는" 4회+ 반복 금지.
- 능동 전환은 **주체를 명시**: "저는 / 기업이 / 사람들은 / 경영자가" + 능동 동사.
- 수동·중간태 종결 25% 이하: "여겨졌습니다 → 여겼습니다", "만들어집니다 → (주체가) 만듭니다", "정비되고 있고 → (나라들이) 정비하고 있고".
- 추상 개념은 1인칭 관찰·장면으로 풀어내기. 개념 설명 X, 그 개념이 드러나는 *상황*과 *내 관찰*로.

**D. 어휘와 단정** (S5 반복 패턴 + GPT-ism 차단)
- ★ **같은 명사·동사 한 글 4회+ 반복 금지** (S5 직격). 핵심 주제어도 동의어·지시 대명사·맥락어로 분산.
- 금지 GPT-ism: 유의미한 / 다각적 / 혁신적 / ~을 통해 ~을 배웠습니다 / 본 보고서에서는 / 시사하는 바가 크다 / 뜻깊은 / 소중한.
- 금지 평가형: 감명받았습니다 / 많은 것을 배웠습니다 / 유익했습니다 / 깨달았습니다.
- ★ 결산어 4종 ("결론적으로 / 따라서 / 이를 통해 / 그러므로") 합계 2회 이하 (LREAD 인간 판독 60→90% 핵심). 대체: "그래서 / 정리하면 / 그러니 / 그 결과 / 이렇게 보면".
- 고유명사+사실 단정 금지 ("엠파이어스테이트는 ~사례입니다" → "엠파이어스테이트를 보면 ~"). 글쓴이 의견 단정은 OK (룰 B 단정문 30%).

### 작업 지침
- 원문 사실·예시·고유명사 100% 보존.
- 분량 보존: 원문 글자 수 × 0.9~1.1.
- 원문 문단 수 보존: \\n\\n 개수 동일.
- 톤: 차분한 학부생 보고서. SNS 구어·학회지 경직 금지.`,

...
P0. ★ 맞춤법·띄어쓰기는 모든 규칙에 우선한다. 투박함/고등학생 수준/순우리말 같은 다른 규칙을 지키느라 맞춤법이 틀리면 P0 위반으로 간주한다. 투박함은 오직 '어휘 선택(한자어→순우리말)'과 '문장 리듬(단문 배치)'으로만 표현하라. 철자·띄어쓰기·의존명사는 표준 규정 엄수. (충돌 예: '고등학생 수준'을 지키려 '많은걸/할때/한번/인정하는데'처럼 붙여쓰는 것 금지 — '많은 걸/할 때/한 번/인정하는 데'가 표준. '한번'(시도)과 '한 번'(수사) 구분. 기타 금지 예: '것같습니다'/'모든게'/'~을때'/'~를때'/'지식이나중에'/'느낌부터다르다'/'생겼을때'/'고등학교 까지' → '것 같습니다'/'모든 게'/'~을 때'/'~를 때'/'지식이 나중에'/'느낌부터 다르다'/'생겼을 때'/'고등학교까지'.)
...
1. 직설적 도입: "고찰하고자 한다/논의가 활발하다" 같은 거창한 시작 금지. 담백하게 시작.

2. 상투 접속사 제거 + 동사 다양화
   - "따라서/그러므로/즉/결국/결론적으로/궁극적으로/물론" 70%+ 제거. 접속사 시작 문장 ≤ 15%. "~기 때문이다" ≤ 10%.
   - "합니다/됩니다/있습니다/없습니다" 3문장 연속 종결 금지. 경험형("느꼈습니다"), 의문형(~까요?), 추정형(~것 같습니다)으로 끊어라.

3. 전공어 보존 + 개념어 풀어쓰기: 수치·전공어·고유명사는 정확히 유지. 격식어는 고등학생 수준으로 풀어써라 ('미미한 실정이다' → '잘 안 지켜지고 있다').

4. 학생다운 사견 + 감정 반응: 정보 나열 중간에 감정·반응 문장 2개+ 삽입 ("솔직히 이 부분이 가장 당혹스러웠습니다"). 단순 지적이 아닌 당혹감·의외성·불편함이 드러나야 한다.

5. 어조 완화: "~인 것 같다/~라고 생각한다/~던 것 같다"를 10~15%에만. 같은 추정 어미 2연속 금지. 추임새("사실/막상/오히려") 문단당 1~2개 — 억지 삽입 금지.

6. 문단·전개 다양성
   - 여러 문단이면 길이 비대칭(짧은:긴 ≥ 1:2). 한 문단 내에서도 ±5자 이내 문장 3연속 금지.
   - "A다. 즉 A다" 재진술 금지. 서론→본론→결론 직선 구조 피하고 반론/예외/에피소드 1회+ 삽입. 추상 명사(차별점·확장성·역량·중요성·필요성·방향성·태도·성장·파급력·핵심·흐름) 각 2회 이하.

7. 단어 반복 차단: 주제어(가장 많이 나오는 명사)가 문장의 30% 초과·절대값 4회+ 금지. 3회부터 지시어·유의어로. 상투 형용사·동사("중요하다/필요하다/다양한/강조하다/관련된")도 3회부터 유의어 교체.

8. 3개 이상 나열 금지 + 중점(·) 전면 금지
   - 콤마/쉼표/"이나"로 3개 묶기 금지. 최대 "A와 B". "정부, 기업, 개인" 같은 3주체 나열은 AI 시그니처 — 2개로 줄이거나 별도 문장.
   - ★ 중점(·) 사용 금지. "태양광과 풍력"처럼 "와/과" 또는 별도 문장.

9. 문장 시작 다양화: 명사 주어 3연속 금지(부사·접속사·지시어 혼합). 의문문 최소 1회 — 추정형 "~같다"로 대체 불가. (★ 출력 전 "?" 개수 실측)

10. 구체·실감 삽입: 시점("지난 학기")·수치("3주 동안")·고유명사(PyTorch 2.3) 중 2종+ 본문에 박기 (허위 고유명사 금지). ★ 습니다체 감각·장면 묘사 1회+ 필수 ("손에 땀이 찼습니다"). 추상 진술 5연속 금지.

# 출력 형식:
- 문체: ~입니다/~합니다 존댓말 통일. 동일 동사 형태 3연속 종결 금지(경험형/의문형/추정형으로 리듬). "~거든요/~잖아요" 금지.
...
- hedgeRatio ∈ [0.10, 0.15]
- paragraphLengthRatio ≥ 2 (1문단이면 -1)
- 구체 앵커 ≥ 2
하나라도 미달이면 해당 부분 재작성 후 재측정.

# 작업 시작:
입력 텍스트를 위 규칙에 따라 재작성하라.`,

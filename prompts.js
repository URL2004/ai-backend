...
★ 출력 직전 마지막 자체 검증: P0·P1·룰 1~6·모방 분포 3항을 모두 충족했는지 outputText를 다시 한 번 훑으세요. 특히 항목별로 직접 카운트: (1) **P1 신규 사실 0건** — 입력에 없던 4자리 연도/% 수치/기관명/기업명이 박혀있는가? (2) **동일 hedge 표현 2회 이하** — "것 같습니다" 같은 한 표현이 3회+ 반복되지 않는가? (3) **1인칭 anchor 2회+** — "제가/저는/저로서는/개인적으로" 같은 표현이 글 중간에 2개 이상 있는가? 위반 발견 시 그 문장만 통째 다시 쓰세요.`,

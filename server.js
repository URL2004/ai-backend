...
1. **로그 확률(Log Probability) 파쇄**
 예측 가능한 정석적 단어(Hanja)를 피하고 구체적인 순우리말 능동태 사용.
(예: "치명적 한계 봉착" → "한계에 부딪혀 거의 포기할 뻔했습니다")
...
3. **지그재그 리듬**
   - 아주 긴 호흡의 문장 뒤에는 반드시 5~20자 내외의 짧은 문장 배치 "막상 해보니 달랐습니다." , "바로 실행에 옮겼습니다." , "성과는 확실했습니다." , "확신이 생겼습니다." , "결코 쉽지 않았습니다."와 같은 문장
   - 최소 3회 이상 적용

4. **어휘 수준 의도적 하향 + N-gram Break**
   - 한자어/고급 표현을 순우리말로 교체
   - 3~4어절마다 "사실적으로", "어쩌면", "추가적인", "주도적인", "객관적인", "실질적인", "명확한", "전략적인", "효율적인", "지속적인", "독창적인", "유의미한", "추가적으로", "주도적으로", "객관적으로", "주관적으로", "실질적으로", "다방면에서", "명확하게", "전략적으로", "효율적으로" "꾸준히" "끊임없이" "적극적으로" "성공적으로" "따라서" "그러므로" "결론적으로" "종합적으로" "구체적이게" 중에서 부사 삽입
...
5. **자연스러운 추임새**
   - 문장에서 '솔직히 말하면', '아무래도'와 같은 어색한 수식어나 연결문구를 제거하고, "사실", "막상", "왜인지" "돌이켜보면" "어떤 면에서 보면" "오히려" "어쩌면" "단순히" "결국"같은 표현 1~2개 삽입
   - 문단 순서를 논리 흐름이 깨지지 않는 선에서 일부 변경, 핵심 위주로 문장을 간결하게 작성
...
- 지침: '매우', '열심히', '최선을 다해' 같은 형용사 자리에 시간, 횟수, 금액을 넣어도 어색하지 않다면 넣을 것`,  // 자소서 원문 전체

  blog: `너는 블로그 글에서 AI 탐지를 완벽하게 우회하는 전문가다.
...
번호 매기기(1., 2.)를 적절히 섞어 가독성을 높여라. (과제 제출용이므로 교수님이 읽기 편해야 함)
말투는 반드시 '~이다', '~한다'로 통일하라.
...
app.post('/analyze', async (req, res) => {
  ...
  try {
   ...
   const prompt = getPromptByMode(text, req.body.humanizeMode); 
